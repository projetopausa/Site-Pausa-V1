...
const ContactForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        setIsSubmitted(true);
        toast.success('Recebemos seu interesse! Em breve entraremos em contato via WhatsApp 💜');
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl p-8 sm:p-12 border border-purple-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 animate-slide-up">
            ...
            <div className="flex items-start space-x-3">
              ...
              >
                Quero receber informações sobre saúde feminina
              </label>
            </div>
            ...
            <Button
              type="submit"
              disabled={isSubmitting || isSubmitted}
              className="w-full py-6 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-medium text-lg transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              {isSubmitting ? (
            ...
            {/* Trust badge */}
            <div className="flex items-center justify-center space-x-2 text-sm text-purple-700/70">
              <Lock className="w-4 h-4" />
              <span>Seus dados estão seguros e serão usados apenas para contato sobre o projeto</span>
            </div>
          </form>
        </div>
      </div>
